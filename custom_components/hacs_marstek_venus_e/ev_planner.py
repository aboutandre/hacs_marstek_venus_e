...
Safety / anti-wear stance:
  - never charge the car from the home batteries (handled by EV-exclusion on the battery side);
...
class EvPlan:
    ...
    phases: int         # 1 or 3
    state: str          # off | not_connected | full | fast | cheap | solar | waiting | hold
    reason: str
    target_power_w: int
...
class EvChargePlanner:
    ...
    def plan(self, obs: EvObservation) -> EvPlan:
        ...
        if available < min_power:
            return self._maybe_stop(obs, "waiting", "insufficient PV surplus")
